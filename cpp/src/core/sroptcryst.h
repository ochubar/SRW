 ...
 * @version 1.0
 * @author A. Rodriguez-Fernandez, I. Petrov
 * @version 2.0 #Laue
 ***************************************************************************/
...
class srTOptCryst : public srTGenOptElem {
	...
	int m_uc; // crystal use case: 1- Bragg Reflection, 2- Bragg Transmission (Laue cases to be added)
               // case: 3- Laue, 4- Laue Transmission ARF08052019 change char to int
...
public:
	...
	srTOptCryst(const SRWLOptCryst& srwlCr)
	{
		...
		m_thicum = srwlCr.tc * 1e+06; //assuming srwlCr.tc in [m] goes to [um]
...
**/
	...
	int PropagateRadiation(srTSRWRadStructAccessData* pRadAccessData, srTParPrecWfrPropag& ParPrecWfrPropag, srTRadResizeVect& ResBeforeAndAfterVect) //virtual in srTGenOptElem
	{
		...
		else
		{
			...
			//double hn = m_HXAi[1]; //OC180314: fix after conversation with A. Suvorov
			//double ht = m_HXAi[2];
			FindDefOutFrameVect(pRadAccessData->avgPhotEn, m_HXAi[1], m_HXAi[2], v_nv, m_tv, m_sv, vZ1, vX1); //Introduce v_nv
			FindRXtRef(vZ1, vX1, m_RLabXt, m_RXtRef);
		}
	...
	{// E in eV; Length in m !!!
		...
		//ARF23052019 Change definition of bee
		//double gamma0;
		//double gammaH;
		//double bee;
        
		//Bragg
		//gamma0 = sin(pi/2 + thBrd + alphrd);
		//gammaH = sin(-pi/2 + thBrd - alphrd);
		//Laue
		//if ((m_uc == 3) || (m_uc == 4))
		//{
		//    gamma0 = sin(pi/2 - thBrd + alphrd);
		...
		complex<double> Cph1C, Cph2C;
		Cph1C = exp(ph1C);
		Cph2C = exp(ph2C);
        
		complex<double> DHsgC,  D0trsC;
        ...
        else if(m_uc == 3) //Laue Reflection LD
		{
			complex<double> Ehx = sgH[0]*EHSPCs + piH[0]*EHSPCp;
			complex<double> Ehy = sgH[1]*EHSPCs + piH[1]*EHSPCp;
			complex<double> Ehz = sgH[2]*EHSPCs + piH[2]*EHSPCp; //Longitudinal component not used (?)
		...
		else if(m_uc == 4) //Laue Transmission (i.e. Forward Bragg Diffraction (FLD))
		{
			//DEBUG
			//complex<double> DHsgC_p_D0trsC = DHsgC + D0trsC;
			//complex<double> DHsgCae2_p_D0trsCae2 = DHsgC.real()*DHsgC.real() + DHsgC.imag()*DHsgC.imag() + D0trsC.real()*D0trsC.real() + D0trsC.imag()*D0trsC.imag();
